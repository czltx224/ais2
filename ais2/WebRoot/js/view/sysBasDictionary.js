...
var jsonread;
var gridLoadMask;
var dictionaryUrl='sys/dictionaryAction!list.action';
var dictionaryDetailUrl='sys/basDictionaryDetailAction!list.action';
var saveDictionaryUrl='sys/basDictionaryDetailAction!save.action';
var delDictionaryUrl='sys/basDictionaryDetailAction!delete.action';
...
//    height: Ext.lib.Dom.getViewHeight(),
    ...
    initComponent: function() {
        ...
        dateStore = new Ext.data.Store({
                proxy: new Ext.data.HttpProxy({url:sysPath+"/"+this.dictionaryDetailUrl}),
                baseParams:{pageSize: countOfPage},
                reader:jsonread
            });
...
Ext.onReady(function(){
  ...
  var  form1 = new Ext.form.FormPanel({
     ...
     var fields=[{name:"id",mapping:'id'},'typeName','typeCode','basDictionaryId','basDictionaryName'];
        ...
        var formItems=[
        	              {fieldLabel: 'ID', id:'id', name: 'id',hidden:true},
        				  {xtype : 'combo',
        				   model : 'remote',
        				   fieldLabel: '类别名称',
				           allowBlank : false,
        				...
        				{fieldLabel: '权限码', name: 'privilege',hidden:true,value:18}
		                ];

        var columns= [
        			sm,
        		    {header: 'ID', dataIndex: 'id'},
        			{header: '条目名称', dataIndex: 'typeName'},
			        {header: '类别编码', dataIndex: 'typeCode'},
   ...
   function deleteForm(){
	...
	function searchLog() {
		  dateStore.proxy = new Ext.data.HttpProxy({
					method : 'POST',
					url : sysPath+'/'+dictionaryDetailUrl
		  });
