...
Ext.onReady(function(){
	...
	var expander = new Ext.grid.RowExpander({
		tpl : new Ext.Template(//'<p style=margin-left:70px;><span style=color:Teal;>项目ID</span><br><span>{xmid}</span></p>',
				'<p style=margin-left:70px;><span style=color:Teal;>异常描述:</span><br><span>{exceptionDescribe}</span></p>',
				'<p style=margin-left:70px;><span style=color:Teal;>异常反馈人:</span><span>{createName}</span></p>'),
		// 设置行双击是否展开
	...
	var fields=[{name:"id",mapping:'id'},'dno','cusName','flightMainNo','flightNo','subNo',
										...
										'finalResult','dealName','dealTime','dealReasult','isCp','isCus','exceptionMoney',
		...
		tbar:[{text:'<b>导出</b>',iconCls:'sort_down',handler:function() {
							...
							parent.Ext.Msg.confirm(alertTitle,
									'修改数据只能选择一数据，请确认您是否真的修改', function(btnYes) {
										if (btnYes == 'yes' || btnYes == 'ok'
												|| btnYes == true) {
											recordGrid.getSelectionModel().clearSelections();
										}
									});
						} else {
							updateException(_records);
						}
				}
			},'-',{
				text:'<b>删除异常</b>',
			...
			//	store : flightStore,
    		...
    		},'-','&nbsp;&nbsp;',
    		  {
	   		...
	   		plugins : [expander],
			columns:[expander,rownum,sm,
			        {header: '配送单号', dataIndex: 'dno',width:60,
			        	renderer:function(v){
			        		//	return v+"<img src='imags/001(1).png' />";
			        		return v;
			        	}
			        },
			        ...
			        {header: '异常描述', dataIndex: 'exceptionDescribe',width:150},
			        ...
			        {header: '异常反馈人', dataIndex: 'createName',width:80},
			...
			if(dNo==''){
					...
					if (Ext.getCmp('comboselect').getValue() == 'EQL_dutyDepartId'){	
		    			Ext.apply(dataStore.baseParams={
							filter_LIKES_exceptionType1: exceptionType,
							filter_LIKES_exceptionNode:nodeName,
							filter_GED_createTime : start,
			    		    filter_LED_createTime : end,
							filter_LIKES_dutyDepartId:Ext.getCmp("comboTypeDepart").getValue(),
						    limit : pageSize
						});
		    		}else if(Ext.getCmp('comboselect').getValue() == 'EQS_cusName'){
		    			Ext.apply(dataStore.baseParams={
							filter_LIKES_exceptionType1: exceptionType,
							filter_LIKES_exceptionNode:nodeName,
							filter_GED_createTime : start,
			    		    filter_LED_createTime : end,
							filter_EQS_cusName:Ext.getCmp("company").getRawValue(),
						    limit : pageSize
						});
		    		}else {
						Ext.apply(dataStore.baseParams={
    ...
    function updateException(_record) {
    		if(_record!=null){
				var node=new Ext.tree.TreeNode({id:'keel_'+_record[0].data.dno,leaf :false,text:"异常处理"});
		      	node.attributes={href1:'exception/oprExceptionAction!showForm.action?id='+_record[0].data.id+'&dno='+_record[0].data.dno};
