//代理协议价管理js
var privilege=91;//权限参数
var gridSearchUrl="fi/cqCorporateRateAction!ralaList.action";//查询地址
var saveUrl="fi/cqCorporateRateAction!save.action";//保存地址
var delUrl="fi/cqCorporateRateAction!deleteStatus.action";//删除地址
var auditUrl="fi/cqCorporateRateAction!auditStatus.action";//审核地址
...
var  fields=[{name:"id",mapping:'id'},
	...
	var tbar=[{text:'<b>导出</b>',iconCls:'sort_down',handler:function() {
                	parent.exportExl(cqCorporateRateGrid);
        ...
        '-',{text:'<b>审核</b>',iconCls:'groupPass',id:'bascqCorporateRateaudit',tooltip : '审核代理协议价',handler:function() {
                	var cqCorporateRate =Ext.getCmp('cqCorporateRateCenter');
					var _records = cqCorporateRate.getSelectionModel().getSelections();
		...
		{text:'<b>新增</b>',iconCls:'userAdd',id:'bascqCorporateRateAdd',tooltip : '新增代理协议价',handler:function() {
                	savecqCorporateRate(null);
            } },'-',
	 	{text:'<b>修改</b>',iconCls:'userEdit',id:'bascqCorporateRateEdit',disabled:true,tooltip : '修改代理协议价',handler:function(){
	 	var cqCorporateRate =Ext.getCmp('cqCorporateRateCenter');
		var _records = cqCorporateRate.getSelectionModel().getSelections();
	 	...
	 	savecqCorporateRate(_records);}},'-',
	 	{text:'<b>删除</b>',iconCls:'userDelete',id:'bascqCorporateRateDelete',disabled:true,tooltip : '删除代理协议价',handler:function(){
	 		var cqCorporateRate =Ext.getCmp('cqCorporateRateCenter');
			var _records = cqCorporateRate.getSelectionModel().getSelections();
...
Ext.onReady(function() {
   ...
   var sm = new Ext.grid.CheckboxSelectionModel({});
    ...
    cqCorporateRateGrid = new Ext.grid.GridPanel({
        renderTo: Ext.getBody(),
        id:'cqCorporateRateCenter',
	    ...
	    cqCorporateRateGrid.on('click', function() {
	       selabled();
	     	
	    });
		cqCorporateRateGrid.on('rowdblclick',function(grid,index,e){
		var _records = cqCorporateRateGrid.getSelectionModel().getSelections();
   ...
   function searchcqCorporateRate() {
		...
		var editbtn = Ext.getCmp('bascqCorporateRateEdit');
		var deletebtn = Ext.getCmp('bascqCorporateRateDelete');
...
function savecqCorporateRate(_records) {
											...
											items : [{
														...
														items : [{
																...
																{xtype : "combo",
																	...
																	mode : "remote",// 从本地载值
																	//valueField : 'startAddrId',// value值，与fields对应
																	...
																	store : endAddrStore,
																	mode : "remote",// 从本地载值
																	//valueField : 'endAddrId',// value值，与fields对应
		...
		cqCorporateRateTitle='添加代理协议价信息';
		if(cid!=null){
		cqCorporateRateTitle='修改代理协议价信息';
		//departStore.load();
 ...
 function importExl(){
				...
				handler : function() {
					if (form2.getForm().isValid()) {
						...
						form2.getForm().submit({
							url : sysPath+ '/fi/cqCorporateRateAction!saveExcel.action',
							method : 'post',
							waitMsg : '正在处理数据...',
							success : function(form, action) {	
								win.hide(), 
	...
	function upLoadFile(){
		window.location.href=cqCorporateRateExcelUrl;
	}
 ...
 function selabled(){
 	 var _record = cqCorporateRateGrid.getSelectionModel().getSelections();
 	
        var updatebtn = Ext.getCmp('bascqCorporateRateEdit');
        var deletebtn = Ext.getCmp('bascqCorporateRateDelete');
        if (_record.length==1) {       	
            if(updatebtn){
