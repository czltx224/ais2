...
// Public method for encoding
	return {encode:(typeof btoa == "function") ? function (input) {
		return btoa(input);
	} : function (input) {
		var output = "";
...
// Calculate cell data types and extra class names which affect formatting
	...
	var colXml = "";
	var headerXml = "";
	for (var i = startColumn; i < cm.getColumnCount(); i++) {
		if (includeHidden || !cm.isHidden(i)) {
			var w = cm.getColumnWidth(i);
			totalWidthInPixels += w;
			colXml += "<ss:Column ss:AutoFitWidth=\"1\" ss:Width=\"" + w + "\" />";
			headerXml += "<ss:Cell ss:StyleID=\"headercell\">" + "<ss:Data ss:Type=\"String\">" + cm.getColumnHeader(i).replace("<br>", " ") + "</ss:Data>" + "<ss:NamedCell ss:Name=\"Print_Titles\" /></ss:Cell>";
	...
	var worksheetName = this.title;
	try{
		if (worksheetName.length > 20) {
...
// Generate worksheet header details.
	var t = "<ss:Worksheet ss:Name=\"" + worksheetName + "\">" + "<ss:Names>" + "<ss:NamedRange ss:Name=\"Print_Titles\" />" + "</ss:Names>" + "<ss:Table x:FullRows=\"1\" x:FullColumns=\"1\"" + " ss:ExpandedColumnCount=\"" + visibleColumnCount +
//'" ss:ExpandedRowCount="' + (this.store.getCount() + 2) + '">' +
	"\" >" + colXml + "<ss:Row ss:Height=\"30\">" + "<ss:Cell ss:StyleID=\"title\" ss:MergeAcross=\"" + (visibleColumnCount - 1) + "\">" + "<ss:Data xmlns:html=\"http://www.w3.org/TR/REC-html40\" ss:Type=\"String\">" + "<html:B>" + (this.title?this.title:"") + "</html:B></ss:Data><ss:NamedCell ss:Name=\"Print_Titles\" />" + "</ss:Cell>" + "</ss:Row>" + "<ss:Row ss:AutoFitHeight=\"1\">" + headerXml + "</ss:Row>";
	
	
		var previous = store.data.items[0].data[cm.getDataIndex(2)];
		var temp_t = "";
		var item_number = 0;
...
// Generate the data rows from the data in the Store
	for (var i = 0, it = this.store.data.items, l = it.length; i < l; i++) {
		...
		r = it[i].data;
		var k = 0;
		for (var j = startColumn; j < cm.getColumnCount(); j++) {
